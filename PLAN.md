...
- Apple Music: 起動 / 終了 50 回、100 曲連続切替、実楽曲の 1 時間連続再生、一時停止、再開、最小化からの追従を確認。Release ビルドは 0 warnings / 0 errors。
- パフォーマンス: 1 時間再生で Private WS 39.3→39.1 MB、CPU 最大 0.0488%。100 曲切替は初期キャッシュ確保後に横ばい、50 回の起動 / 終了でも Private Bytes に増加傾向なし。
- Windows 統合: Explorer 再起動、タスクバー自動非表示 / 再表示、全画面アプリ、ライト⇄ダーク、トレイアイコン再登録、340→136 DIP のコンパクト表示、最小幅未満での非表示を確認。
- 品質確認で発見した不具合を修正: Explorer 再起動後のトレイアイコン消失、全画面検出漏れ、初回衝突時に帯が既定位置へ表示される問題。
- 未確認: スリープ / 復帰、ロック / ログイン、モニター抜き差し、解像度変更、DPI 125 / 150 / 200%、Artwork なし、長いメタデータ、1 分未満 / 1 時間以上の楽曲。物理操作または該当データが必要なため、最終手動確認へ持ち越す。
